...
"""
Finds the org ID of a Webex user given their email -- handy for users outside your own org,
whose org ID the People API will no longer return without already knowing it.

A space membership record exposes each member's org ID, so this looks for an existing 1:1
direct space with the user and reads it from there. If none exists, it offers to create one
(which posts a message to the user) and reads the org ID from that.

The org ID is the only thing written to stdout, so the result can be piped or captured. All
...
def org_id_from_existing_direct(api, email):
    """find an existing 1:1 space with the user and return their org id, or None
    ...
    for count, room in enumerate(
        ...
        try:
            for membership in api.memberships.list(roomId=room.id, personEmail=email):
                if (membership.personEmail or '').lower() == target and membership.personOrgId:
                    return membership.personOrgId
        except ApiError:
...
def org_id_from_new_direct_space(api, email):
    """create a 1:1 space with the user; return (org_id, room_id), cleaning up the message
    ...
    for membership in api.memberships.list(roomId=room_id, personEmail=email):
        if membership.personOrgId:
            org_id = membership.personOrgId
            break
    ...
    return org_id, room_id

def main():
    """find and print the org id for the supplied user email"""
    ...
    room_id = None
    org_id = org_id_from_existing_direct(api, user_email)

    if org_id is None:
        print(f'No 1:1 space with {user_email} found in the {DIRECT_SCAN_LIMIT} most recently '
              'active direct spaces.', file=sys.stderr)
        if not confirmed(f'Create a 1:1 space with {user_email} to look up their org? '
                         '(this posts a message to them)'):
            print('### Aborted; no org id retrieved. ###', file=sys.stderr)
            sys.exit(1)
        print(f'Creating direct space with {user_email}...', file=sys.stderr)
        try:
            org_id, room_id = org_id_from_new_direct_space(api, user_email)
        except ApiError as error:
    ...
    if not org_id:
        print(f'### Could not determine an org id for {user_email}. ###', file=sys.stderr)
        sys.exit(1)

    # the org id is the only thing on stdout (the result this utility exists to produce), so it
    # can be piped or captured cleanly; any created space's room id is logged to stderr above
    print(org_id)
